...
#define closest_word_count  ((long long)40)                  // number of closest words that will be shown
...
int main(int argc, char** argv) {
	FILE* f;
	char st1[max_size];
	char bestw[closest_word_count][max_size];
	...
	while (1) {
		...
		for (a = 0; a < cn; a++) {
			for (b = 0; b < words; b++) if (!strcmp(&vocab[b * max_word_len], st[a])) break;
			if (b == words) b = 0;
			bi[a] = b;
