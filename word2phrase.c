...
void LearnVocabFromTrainFile() {
	...
	while (1) {
		...
		//
		strcpy_s(last_word, sizeof(last_word), word);
...
void TrainModel() {
	...
	errno_t err1 = fopen_s(&fin, train_file, "rb");
	errno_t err2 = fopen_s(&fo, output_file, "wb");
	if (fin == NULL || NULL == fo || 0 != err1 || 0 != err2) {
		printf("ERROR: training data file not found!\n");
		exit(1);
	}
...
int main(int argc, char** argv) {
	...
	if ((i = ArgPos((char*)"-threshold", argc, argv)) > 0) threshold = (real)atof(argv[i + 1]);
